...
class LiveNodePublisherConfig(override val config: Config) extends PublishConfig(config, "live-node-publisher") {
  ...
  val liveVideoStreamTopic: String = config.getString("kafka.live_video_stream.topic")
  ...
  val inputConsumerName = "live-node-publisher-consumer"
  ...
  val extractableMimeTypes = List("application/vnd.ekstep.ecml-archive", "application/vnd.ekstep.html-archive", "application/vnd.ekstep.plugin-archive", "application/vnd.ekstep.h5p-archive", "application/vnd.ekstep.scorm-archive", "application/vnd.ekstep.qti-archive")
