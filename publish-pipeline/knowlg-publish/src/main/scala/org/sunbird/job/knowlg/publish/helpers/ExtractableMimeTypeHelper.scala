...
import org.sunbird.job.knowlg.publish.processor.{JsonParser, Media, Plugin, XmlParser}
...
object ExtractableMimeTypeHelper {
  ...
  def getCloudStoreURL(obj: ObjectData, cloudStorageUtil: CloudStorageUtil, config: KnowlgPublishConfig): String = {
  ...
  private def getExtractionPath(obj: ObjectData, config: KnowlgPublishConfig, suffix: String): String = {
    obj.mimeType match {
      ...
      case "application/vnd.ekstep.qti-archive" => config.contentFolder + File.separator + "qti" + File.separator + obj.identifier + "-" + suffix
      case _ => ""
    }
  }
  ...
  def copyExtractedContentPackage(obj: ObjectData, contentConfig: KnowlgPublishConfig, extractionType: String, cloudStorageUtil: CloudStorageUtil): Unit = {
  ...
  def processECMLBody(obj: ObjectData, config: KnowlgPublishConfig)(implicit ec: ExecutionContext, cloudStorageUtil: CloudStorageUtil): Map[String, AnyRef] = {
  ...
  private def extractPackageInCloud(uploadFile: File, obj: ObjectData, extractionType: String, slugFile: Boolean, basePath: String, config: KnowlgPublishConfig)(implicit cloudStorageUtil: CloudStorageUtil) = {
  ...
  private def uploadArtifactToCloud(uploadedFile: File, identifier: String, filePath: Option[String] = None, config: KnowlgPublishConfig)(implicit cloudStorageUtil: CloudStorageUtil): Array[String] = {
    val urlArray = {
      try {
  ...
  private def validationForCloudExtraction(file: File, extractionType: String, mimeType: String, config: KnowlgPublishConfig): Unit = {
  ...
  private def localizeAssets(contentId: String, ecrfObj: Plugin, basePath: String, config: KnowlgPublishConfig)(implicit ec: ExecutionContext, cloudStorageUtil: CloudStorageUtil): Unit = {
  ...
  private def processAssetsDownload(contentId: String, medias: List[Media], basePath: String, config: KnowlgPublishConfig)(implicit ec: ExecutionContext, cloudStorageUtil: CloudStorageUtil): Map[String, String] = {
  ...
  private def downloadAssetFiles(identifier: String, mediaFiles: List[Media], basePath: String, config: KnowlgPublishConfig)(implicit ec: ExecutionContext, cloudStorageUtil: CloudStorageUtil): Future[List[Map[String, String]]] = {
